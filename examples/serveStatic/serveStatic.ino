...
void setup(void){
  ...
  Serial.print("\nStarting ethernet web server demoing 'serveStatic' function on " + String(BOARD_TYPE));
  ...
  IPAddress eth_IP(192, 168, 1, 122);    // *** CHANGE THIS to something relevant for YOUR LAN. ***
  IPAddress eth_MASK(255, 255, 255, 0);   // Subnet mask.
  IPAddress eth_DNS(8, 8, 8, 8);    // *** CHANGE THIS to match YOUR DNS server.           ***
  IPAddress eth_GW(192, 168, 1, 5);   // *** CHANGE THIS to match YOUR Gateway (router).     ***
  Ethernet.init(SS);           // GPIO5 on ESP32, resets W5500 board.
...
void initSPIFFS(void) {
  ...
  if (!SPIFFS.begin()) {
    Serial.println("Failed to initialise SPIFFS module.");
  }
}
